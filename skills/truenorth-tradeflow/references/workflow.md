...
- Version `0.1.3` transitions from `AWAITING_USER_APPROVAL` only to `REVIEW_COMPLETE`, `CANCELLED`, or `EXPIRED`. It never invokes the order panel. A future live state may be added only after independent mapping proves exact control semantics and matching order/position read-back.
