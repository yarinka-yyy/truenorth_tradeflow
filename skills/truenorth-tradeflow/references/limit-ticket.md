...
- **Reduce only** off;
- **Take Profit / Stop Loss** on with exact levels from the completed setup or current user choice, or off only when that source explicitly says `none`;
- **Skip Open Order Confirmation** off; and
- **Attach an agent** off.

If GTC, the requested limit price, the requested amount target, or chosen protection state cannot be rendered, stop and ask for one new current choice. Do not use **Customize**, **Start watching**, or an attached agent.

Read the current ticket after setting it. Capture market, direction, Limit type, limit price, leverage, TIF, Size unit/value, Order Value, Margin Required, fees, TP/SL and their displayed gain/loss if enabled, liquidation estimate, the three opening-control states, and the submit label.
...
Immediately before the outer click, re-read the core intent. If market, direction, order type, exact limit price, requested leverage or amount target, protection, TIF, or opening-control state cannot match the approval, pause for a new current choice. If the editable ticket's Margin Required drifted from the requested target, adjust Size until it matches again; stop if it cannot. Do not request another approval merely because the live quote, rounded base size, fees, liquidation, or final labels refresh while the approved Limit intent remains the same.
...
2. Continue under the existing approval only when it still represents the same market, direction, Limit opening, exact price, submitted amount intent, GTC intent, and chosen protection state. The final window may show converted token size without the original USDC Size input; do not require an unrendered field. A calculated Order Value or Margin Required refresh alone does not change intent; stop if the final values show a changed input, the order became unavailable, or the final layer is unclear.
...
After the final attempt, read the selected account's exact-market **Positions** and **Open Orders** rows and compare them with the saved baseline. A button click, toast, confirmation screen, available-balance change, or ticket text is not execution evidence.

- **Open order** — a new approved resting limit entry is rendered relative to the baseline and no new filled position is rendered. With TP/SL enabled, report protective close orders only when the UI actually shows them, with their exact trigger conditions.
- **Opened** — a new exact-market position row is rendered relative to the baseline. Read Open Orders as well and report any protective orders exactly as rendered.
- **Not executed** — neither a new approved entry order nor a new position is rendered after the completed flow.
