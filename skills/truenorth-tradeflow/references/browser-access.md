...
For a local Chrome session already started with `--remote-debugging-port=9222` and a non-default `--user-data-dir`, the user can register that MCP server in Codex CLI:
...
- Navigate only to `https://truenorth.xyz`. Stop on off-origin redirects, popups, or links and let the user handle sign-in, wallet, signature, password, permissions, and 2FA prompts.
