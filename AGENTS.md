...
- Keep the workflow human-led: show the current ticket in plain language, receive one current opening approval that also covers a same-intent rendered final confirmation, then act once and read back the rendered result. Ask again only when the current intent cannot be honored or a protected prompt requires the user.
